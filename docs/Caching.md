...
캐싱이란 <span style="color:red"> 어플리케이션의 성능을 대폭 상승 </span>
시켜주는 대표적인 기술이다.  
...
그럼 어떤 정보를 저장해야 할까? 바로 <span style="color:red"> 가장 많이 조회되는 데이터 </span>
를 저장하면 된다.  
...
웹 어플리케이션에서의 캐싱은 크게 두 가지로 분류된다. 바로 <span style="color:red"> 로컬 캐시 </span>와 <span style="color:red">
글로벌 캐시 </span>이다.
...
여러 어플리케이션 인스턴스나 서버 간에 <span style="color:red"> 데이터를 공유하는 캐시 </span>이다. 이 캐시는 로컬 캐시의 특징을 반대로 갖는다고
생각하면 편리하다.  
...
더불어 특별한 조치를 취하지 않으면, db에서 받은 데이터를 캐시에 N번 기록하는 작업도 진행된다. 이는 <span style="color:red"> DB와 캐시저장소 모두에
부하 </span>를 일으키는 원인이 된다.   
...
예약 데이터를 메모리에 담을 경우 메모리 효율이 떨어집니다.(`부적합`)
...
#### <span style="color:red"> 콘서트 조회 </span> (`요구사항에 존재하지 않는 API`)
...
## 실제 서비스할때를 가정하고, 콘서트 조회(@Cacheable) API와 콘서트 생성(동기화, Evict전략)를 구현
...
`대기열 생성 요청했을 때 대기열이 존재하지 않을 시` 대기열 요청을 보내면 테이블에 대기열을 적재한다.  
`대기열 생성 요청했을 때 대기열이 존재한다면(폴링 겸용)` 해당 대기열의 순번을 조회한다.(PASS상태면 0번)  
`waiting 상태에서 10분이 지나면` 스케줄러에 의해 대기열을 PASS 상태로 전환한다.  
`PASS 상태에서 10분이 지나면` 스케줄러에 의해 대기열을 삭제한다.  
